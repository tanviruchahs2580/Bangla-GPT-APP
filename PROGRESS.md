...
- [x] **0.7 Real deploy + automated smoke** | status: done (script) | evidence: scripts/smoke.sh created (health→register→login→learn→tutor grounded→quiz→me→export→delete, local smoke via python verified, rollback doc in script); staging deploy via release.yml pending server 🖐 | commit: —
