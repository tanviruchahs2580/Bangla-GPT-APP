...
export function SchoolSections() {
  const students = useQuery({ queryKey: ['school-students'], queryFn: () => getSchoolStudents(100, 0) })
  const teachers = useQuery({ queryKey: ['school-teachers'], queryFn: getSchoolTeachers })
  const classes = useQuery({ queryKey: ['school-classes'], queryFn: getSchoolClasses })
  const coverage = useQuery({ queryKey: ['school-coverage'], queryFn: getSchoolCoverage })
              ...
              <tbody>
                {classes.data.map((c) => (
                  <tr key={c.classroom_id}>
                    ...
                    <td>{c.avg_quiz_accuracy == null ? '—' : `${Math.round(c.avg_quiz_accuracy)}%`}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {analytics.data && (
        <section className="card" aria-label={t('anaTitle')}>
          ...
          <div className="stat-grid">
            <div className="stat">
              <span className="stat-value">{Math.round(analytics.data.daily_active_avg)}</span>
              <span className="stat-label">{t('anaDaily')}</span>
            </div>
            <div className="stat">
              <span className="stat-value">{analytics.data.questions_asked}</span>
            ...
            <div className="stat">
              <span className="stat-value">
                {analytics.data.avg_quiz_score_pct == null ? '—' : `${Math.round(analytics.data.avg_quiz_score_pct)}%`}
              </span>
