...
const KIND_LABEL: Record<string, 'searchKindSubject' | 'searchKindChapter' | 'searchKindQuestion'> = {
...
export function SearchBox() {
  ...
  useEffect(() => {
    ...
    get<SearchResponse>(`/search?q=${encodeURIComponent(debounced)}`)
      ...
      .catch(() => {
        if (!cancelled) setData({ query: debounced, ask_action: false, hits: [] })
      })
      .finally(() => {
  ...
  const showDropdown = open && debounced.length >= 2 && !loading && data !== null
  ...
  return (
    <div className="searchbox" ref={boxRef}>
      ...
      />
      {showDropdown && (
        <div className="search-dropdown" role="listbox" aria-label={t('searchPlaceholder')}>
          {data.ask_action && (
            <button
              type="button"
              role="option"
          ...
          {data.hits.map((h, i) => (
            ...
            >
              <span className="search-kind">{t(KIND_LABEL[h.kind] ?? 'searchKindChapter')}</span>
