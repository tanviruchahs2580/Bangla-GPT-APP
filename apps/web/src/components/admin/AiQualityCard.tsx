  ...
  return (
    <section className="card">
      ...
      <p style={{ margin: 'var(--space-2) 0 0' }}>
        <Badge tone="ok">{t('aiqThumbsUp')} {q.thumbs_up}</Badge>{' '}
        <Badge tone={q.thumbs_down > 0 ? 'warn' : 'ok'}>{t('aiqThumbsDown')} {q.thumbs_down}</Badge>{' '}
        <Badge tone={q.refusals_total > 0 ? 'warn' : 'ok'}>{t('aiqRefusals')} {q.refusals_total}</Badge>
      </p>
      {reasons.length > 0 && (
