...
const STRAT_LABELS: Record<
  (typeof STRATEGIES)[number],
  "stratSimple" | "stratExample" | "stratBook" | "stratSteps" | "stratAnalogy"
> = {
  simple: "stratSimple",
...
export default function AITutorPage() {
  ...
  const send = useCallback(
    async (preset?: string, opts?: { reteach?: boolean }) => {
      const text = (preset ?? input).trim();
      if (!text || streaming) return;
      setInput("");
      setError(null);
      track("tutor_question_sent", {
        reteach: opts?.reteach === true,
      ...
      setMessages((m) => [
        ...m,
        { id: 0, role: "user", content: text, grounded: null, sources: [] },
      ]);
      setStreaming(true);
  ...
  const runSearch = async () => {
    ...
    try {
      setHits(
        await get<MessageSearchHit[]>(
          `/tutor/messages/search?q=${encodeURIComponent(q)}`,
        ),
      );
    } catch {
      setHits([]);
  ...
  return (
    <main className="shell-main">
            ...
            renamingId === c.id ? (
              ...
              >
                ...
                <span
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && void removeConversation(c.id)
                  }
                >
      ...
      {evidence && (
        <EvidenceModal source={evidence} onClose={() => setEvidence(null)} />
      )}
    </main>
  );
}
