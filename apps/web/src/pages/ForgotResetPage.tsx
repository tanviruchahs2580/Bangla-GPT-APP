...
export default function ForgotResetPage() {
  ...
  return (
    <div className="card auth-card">
      ...
      <p className="muted">
        <button className={`rate-btn${tab === 'forgot' ? ' on-up' : ''}`} onClick={() => setTab('forgot')}>
          {t('resetTitle')}
        </button>{' '}
        <button className={`rate-btn${tab === 'verify' ? ' on-up' : ''}`} onClick={() => setTab('verify')}>
          {t('verifyTitle')}
        </button>
      </p>
...
function ForgotForm() {
  ...
  if (!requested) {
    return (
      <form onSubmit={requestToken}>
        ...
        <input id="femail" type="email" autoComplete="email" value={email} required onChange={(e) => setEmail(e.target.value)} />
  ...
  return (
    <>
      <p className="ok" role="status">{t('forgotSent')}</p>
      <form onSubmit={doReset}>
        <label htmlFor="rtoken">Reset code</label>
        <input id="rtoken" value={token} required minLength={16} onChange={(e) => setToken(e.target.value)} />
...
function VerifyForm() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(friendlyError((err as { code?: string }).code ?? 'invalid_token'))
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <>
      ...
      <form onSubmit={submit}>
        ...
        <input id="vcode" value={code} required minLength={16} onChange={(e) => setCode(e.target.value)} />
