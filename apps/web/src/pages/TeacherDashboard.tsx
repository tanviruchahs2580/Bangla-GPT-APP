...
export default function TeacherDashboard() {
  ...
  const qpAllReviewed =
    qp !== null &&
    qp.questions.length > 0 &&
    qp.questions.every((q) => q.reviewed);
    ...
    <>
      ...
      <div className="card">
        ...
        <form onSubmit={generateQp} className="grid-2">
          <div>
            ...
            <select
              id="qplvl"
              value={qpLevel}
              onChange={(e) => setQpLevel(Number(e.target.value))}
            >
              {Array.from({ length: 10 }, (_, i) => i + 1).map((n) => (
                <option key={n} value={n}>
                  {n}
        ...
        {qpMsg && (
          <p
            role="status"
            className={qpMsg === t("errorGeneric") ? "error" : undefined}
          >
            {qpMsg}
          </p>
        )}
              ...
              {qp.questions.map((q, idx) => (
                <li key={q.ref} style={{ marginBottom: "12px" }}>
                  <p>
                    ...
                    {q.reviewed && (
                      <span className="badge"> {t("qpReviewAll")}</span>
                    )}
                  </p>
      ...
      <div className="card">
        ...
        >
          ...
          <button
            className="primary"
            disabled={stBusy || roomId === null || !stChapter.trim()}
          >
            {t("stAssign")}
          </button>
        </form>
      ...
      <div className="card">
        ...
        >
          ...
          <button
            className="primary"
            disabled={lpBusy || selected === null || !lpChapter.trim()}
          >
            {t("lpGenerate")}
          </button>
        </form>
            ...
            {Object.entries(lp.sections).map(([key, value]) => (
              <div key={key} style={{ marginBottom: "8px" }}>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setLpEdits((m) => ({ ...m, [key]: e.target.value }))
                  }
                  rows={3}
                  style={{ width: "100%" }}
                />
      ...
      {analytics && analytics.chapters.length > 0 && (
        <div className="card">
          <h2>
            {t("accuracy")} ({t("className").replace(/\s*\(.*\)/, "")}{" "}
            {analytics.class_level})
          </h2>
          <div className="table-scroll">
            <table>
              <thead>
