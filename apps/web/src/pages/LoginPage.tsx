      ...
      <Card className="auth-card">
        ...
        <form onSubmit={submit}>
          <div className="field">
            ...
            <input className="input" id="email" name="email" autoComplete="email" type="email" value={email} required onChange={(e) => setEmail(e.target.value)} />
          </div>
          <div className="field">
            <label htmlFor="password">{t('password')}</label>
            <input className="input" id="password" name="password" autoComplete="current-password" type="password" value={password} required minLength={8} onChange={(e) => setPassword(e.target.value)} />
          </div>
          {error && (
                  ...
                  <a
                    ...
                    onClick={(e) => {
                      ...
                      void import('../api').then(({ resendVerification }) =>
                        resendVerification().then(() => setError({ text: t('verifySent') })),
                      )
                    }}
                  >
