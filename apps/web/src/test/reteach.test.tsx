...
const me = { user_id: 1, email: 's@x.com', role: 'student', profile_id: 7, name: 'S', class_level: 7 }
...
describe('S4.5 re-teach cards in the quiz flow', () => {
  beforeEach(() => {
    ...
    apiMock.get.mockImplementation((path: string) => {
      if (path === '/revision/due') return Promise.resolve({ today: '2026-01-05', due_count: 0, items: [] })
      if (path.includes('/progress'))
        return Promise.resolve({ student: {}, attempts_graded: 0, avg_score_pct: null, by_chapter: [], weak_chapters: [] })
  ...
  it('shows the grounded re-teach card before the first question of a new quiz', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: new RegExp(t('startQuiz')) }))
  ...
  it('shows re-teach cards on the result screen after a wrong answer', async () => {
    apiMock.post.mockImplementation((path: string) => {
      ...
      if (path.endsWith('/submit')) return Promise.resolve(structuredClone(RESULT))
      return Promise.resolve({})
    })
    const user = userEvent.setup()
    renderPage()
    ...
    await user.click(await screen.findByRole('button', { name: new RegExp(t('finishQuiz')) }))

    await waitFor(() => expect(screen.getByText(t('reteachTitle'))).toBeInTheDocument())
    expect(screen.getByText(CARD.excerpt)).toBeInTheDocument()
  ...
  it('renders nothing when the API returns no gaps (no regressions for fresh students)', async () => {
    apiMock.post.mockImplementation((path: string) => {
      if (path === '/quizzes')
        return Promise.resolve({ ...structuredClone(STARTED), reteach: [] })
      if (path.endsWith('/submit')) return Promise.resolve({ ...structuredClone(RESULT), reteach: [] })
      return Promise.resolve({})
    })
    const user = userEvent.setup()
    renderPage()
