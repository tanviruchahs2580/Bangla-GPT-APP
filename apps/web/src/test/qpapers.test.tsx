...
beforeEach(() => {
  ...
  apiMock.get.mockImplementation((path: string) => {
    ...
    if (path.includes('/analytics')) {
      return Promise.resolve({ class_level: 6, students: 2, chapters: [], weak_chapters: [] })
    }
    return Promise.resolve(null)
  })
  apiMock.post.mockImplementation((path: string, body?: unknown) => {
    if (path === '/teacher/qpapers') return Promise.resolve(structuredClone(paper))
    if (path.endsWith('/review')) {
      const decisions = (body as { decisions: { ref: string; action: string; text?: string }[] })
        .decisions
      ...
      if (paper.questions.every((x) => x.reviewed)) paper.reviewed_at = '2026-09-06T00:00:00'
...
describe('S2.4 question paper builder', () => {
  it('generates a draft with the difficulty mix and shows questions', async () => {
    ...
    expect(screen.getByText(new RegExp(t('qpNeedsReview')))).toBeInTheDocument()
  })
  ...
  it('blocks Finalize until every question is reviewed, then finalizes', async () => {
    ...
    const footerAccept = screen.getAllByRole('button', { name: t('qpReviewAll') }).at(-1)!
    ...
    await waitFor(() => expect(apiMock.post).toHaveBeenCalledWith('/teacher/qpapers/1/finalize'))
    await waitFor(() =>
      expect(screen.getAllByText(new RegExp(t('qpFinalized'))).length).toBeGreaterThan(0),
    )
  })
  ...
  it('sends edit decisions with replacement text', async () => {
    const user = userEvent.setup()
    await openQpCard(user)
    await user.type(screen.getByLabelText(`${t('qpTitle')} 2`), 'MY NEW QUESTION')
    await user.click(screen.getAllByRole('button', { name: t('qpReviewAll') })[0])
    await waitFor(() =>
      expect(apiMock.post).toHaveBeenCalledWith(
        '/teacher/qpapers/1/review',
        expect.objectContaining({ decisions: [{ ref: 'q1', action: 'accept' }] }),
      ),
    )
  })
  ...
  it('replaces a single question', async () => {
    const user = userEvent.setup()
    await openQpCard(user)
    await user.click(screen.getAllByRole('button', { name: t('qpReplace') })[0])
    await waitFor(() =>
      expect(apiMock.post).toHaveBeenCalledWith('/teacher/qpapers/1/replace', { ref: 'q1' }),
    )
    await waitFor(() => expect(screen.getByText(/REPLACED q1/)).toBeInTheDocument())
  })
})
