...
describe("S1.8 history UI", () => {
  ...
  it("rename sends PATCH with the new title", async () => {
    ...
    await waitFor(() =>
      expect(apiMock.patch).toHaveBeenCalledWith("/tutor/conversations/5", {
        title: "My-New-Title",
      }),
    );
  });
  ...
  it("delete sends DELETE and the chip disappears from the list", async () => {
    ...
    await user.click(
      await screen.findByRole("button", { name: t("deleteChat") }),
    );

    await waitFor(() =>
      expect(apiMock.del).toHaveBeenCalledWith("/tutor/conversations/5"),
    );
