...
describe('S1.10 revision tab UI', () => {
  beforeEach(() => {
    ...
    apiMock.get.mockImplementation((path: string) => {
      if (path === '/revision/due') return Promise.resolve(structuredClone(DUE))
      if (path.includes('/progress')) return Promise.resolve({ student: {}, attempts_graded: 0, avg_score_pct: null, by_chapter: [], weak_chapters: [] })
      return Promise.resolve(null)
    })
    apiMock.post.mockResolvedValue({ id: 11, reps: 1, interval_days: 1, due_date: '2026-01-06' })
  })
  ...
  it('shows the due count badge and the revision card after switching tabs', async () => {
    ...
    const tab = await screen.findByRole('tab', { name: new RegExp(t('revisionTab')) })
  ...
  it('answering a due item POSTs the review and refreshes the list', async () => {
    ...
    await user.click(await screen.findByRole('tab', { name: new RegExp(t('revisionTab')) }))
    await user.click(await screen.findByText('Option-B'))

    await waitFor(() => expect(apiMock.post).toHaveBeenCalledWith('/revision/11/review', { chosen: 1 }))
    // refetch of the due list happened after the review
    await waitFor(() => {
      const calls = apiMock.get.mock.calls.filter((c: unknown[]) => c[0] === '/revision/due')
