...
import { t } from '../i18n'
import type { AdminSchoolStats, ContentVersionRow, SchoolInviteAdmin } from '../types'
...
beforeEach(() => {
  ...
  apiMock.get.mockImplementation((path: string) => {
    ...
    if (path === '/admin/schools/stats') return Promise.resolve(structuredClone(SCHOOLS))
    if (path === '/admin/content/versions') return Promise.resolve(structuredClone(VERSIONS))
    if (path === '/admin/schools/1/invites') return Promise.resolve(structuredClone(INVITES))
...
describe('S3.5 admin center', () => {
  ...
  it('shows empty hints when there is nothing yet', async () => {
    apiMock.get.mockImplementation((path: string) => {
      ...
      if (path.startsWith('/admin/users')) return Promise.resolve({ total: 0, items: [] })
      return Promise.resolve(null)
    })
    render(<AdminDashboard />)
    await waitFor(() => expect(screen.getByText(t('admNoSchools'))).toBeInTheDocument())
  ...
  it('creates an invite once (plaintext shown once) and revokes an active one', async () => {
    ...
    expect(apiMock.post).toHaveBeenCalledWith('/schools/1/invites', { role: 'teacher' })

    // open the invite list for school 1
    fireEvent.click(screen.getByRole('button', { name: `Adm School ${t('admInvites')}` }))
    ...
    await waitFor(() => expect(apiMock.del).toHaveBeenCalledWith('/admin/schools/1/invites/5'))
    await waitFor(() => expect(screen.getByText(t('admRevoked'))).toBeInTheDocument())
  })
})
