...
describe('S1.11 global search dropdown', () => {
  ...
  it('queries /search after debounce and shows question + chapter results', async () => {
    ...
    await waitFor(() => expect(apiMock.get).toHaveBeenCalledWith('/search?q=kos'), {
      timeout: 2000,
    })
  ...
  it('the ask action navigates to the tutor with the query in state', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('probe')).toHaveTextContent('/student/tutor|{"ask":"kos"}'),
    )
  })

  it('shows the empty state when nothing matches', async () => {
    apiMock.get.mockResolvedValue({ query: 'zzz', ask_action: false, hits: [] })
    const user = userEvent.setup()
    renderBox()
