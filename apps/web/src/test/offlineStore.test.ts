...
function fixture(chapter = 'cell'): ChapterContentOut {
  return {
    ...
    chapter,
    sections: [{ section: 'What is a cell', text: 'Cells are the basic unit of life.' }],
  }
}
...
describe('fetchChapterWithOfflineFallback', () => {
  it('prefers the network when online', async () => {
    ...
    const res = await fetchChapterWithOfflineFallback('science', 'live', 6, async () => live)
  ...
  it('falls back to the saved copy when the network fails', async () => {
    ...
    const res = await fetchChapterWithOfflineFallback('science', 'leaf', 6, async () => {
      throw boom
    })
